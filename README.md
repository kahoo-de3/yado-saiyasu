...
- 空室検索: `VacantHotelSearch/20170426`（hits最大30、page最大100）
- **sortに指定できるのは `standard`（おすすめ順）/ `+roomCharge`（安い順）/ `-roomCharge`（高い順）の3種のみ**。
  レビュー評価順・レビュー件数順は**API非対応**（レスポンスには `reviewAverage` / `reviewCount` が含まれるので、
  クライアント側でなら並び替えられる）
...
- **並び替え**は 安い順 / 高い順 / 星評価の高い順 / クチコミの多い順（v23）。APIのsortが料金順しか
  対応していないため実装が2種類に分かれる:
  **料金順** = APIに並び替えさせる（全件から正しい順で取れる＝正確）。
  **評価系** = `standard` で取得しクライアント側で並び替える＝**取得済みの範囲での並び替え**。
  後者は誤解を招くので、フォームの注記と結果ラベル「（取得分）」で明示している。
  なお複数エリアの結果はマージ後に必ず再ソートする（各エリアの結果を混ぜると順序が崩れるため）
